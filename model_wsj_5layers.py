...
from CTCBeamDecoderLM import ctcdecodelm, n_gram_lm
...
class Model(nn.Module):
    ...
    def __init__(self, input_dim, num_class, CONFIG):
        ...
        self.ngram_lm = n_gram_lm(lm_path='WSJ_Character_7.lm', n_gram=3, character_set='Character_Set.txt')
    ...
    def forward_imp(self, x, softmax=False):
        ...
        if self.reduce_factor == 2:
            batch, seq_len, hid_dim = x.size()
            x = x.view(batch, -1, hid_dim*2)
    ...
    def test_model(self, inputs, labels, beam_size=2, alpha=0.25):
        # Calculate loss and CER together
        self.eval()
        x, y, x_lens, y_lens = self.collate(inputs, labels)
        out, probs = self.forward_imp(x, softmax=True)
        ...
        probs = probs.cpu().detach().numpy()
        x_lens = x_lens.cpu().detach().numpy()
        pred = [ctcdecodelm(p[:l], beam_size=beam_size, blank=self.blank, lm=self.ngram_lm, alpha=alpha)[0] for p, l in zip(probs, x_lens)]
        return loss.item(), pred
